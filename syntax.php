<?php
/**
 * OrphanMedia2 Plugin: Display orphan and missing media files
...
define('DEBUG_MODE_ACTIVE',TRUE);         //to show some information if set to true

/******************************************************************************
 * All DokuWiki plugins to extend the parser/rendering mechanism
 * need to inherit from this class
 */
class syntax_plugin_orphanmedia2 extends DokuWiki_Syntax_Plugin {
/******************************************************************************/
/* return some info
*/
    function getInfo(){
...
*/
    function connectTo($mode) {
        $this->Lexer->addSpecialPattern('~~ORPHANMEDIA:[0-9a-zA-Z_:!;|]+~~',$mode,'plugin_orphanmedia2');
    }
/******************************************************************************/
/* Handle the match
*/
        ...
        echo '</pre>';  */
            ...
            if((DEBUG_MODE_ACTIVE != false) || (stripos($data[0],"perf") !== false)) {
                    $output .= '<div class="xm_perf">
                                  <p style="margin-top:10px;"><img src="'.DOKU_URL.'/lib/plugins/orphanmedia2/images/a1.png" class="a1_img"><b><u>PERFORMANCE MEASUREMENT</u></b></p>'.NL.
...
//            echo sprintf("<p>%s</p>\n", var_dump($listPageFile_MediaLinks));
            ...
            foreach($listMediaFiles[1] as $check) {
                if($check === '0') {
                  ...
                  $output_orphan .= '<tr>'.NL.
                              '<td>'.NL.
                                  '<img src="'.DOKU_URL.'/lib/plugins/orphanmedia2/images/'.$nok_img.'" alt="nok" title="orphan" align="middle" />'.NL.
                              '</td>'.NL.
...
*/
    function _get_allMediaLinks($listPageFiles, $defFileTypes, $skip_NS) {
        ...
        $ignore_tags = array();
        $ignore_tags = parse_ini_file(DOKU_PLUGIN."orphanmedia2/config.ini");
        if (file_exists(DOKU_PLUGIN."orphanmedia2/user_pattern.ini")) $ignore_tags = $ignore_tags + parse_ini_file(DOKU_PLUGIN."orphanmedia2/user_pattern.ini");
...
// ---------------------------------------------------------------
    function _prepare_output($m_link,$page,$img,$counter,$class=NULL)
    {
            ...
            $output.= '<tr>'.NL.
                      '   <td class="col0 centeralign"><img src="'.DOKU_URL.'lib/plugins/orphanmedia2/images/'.$img.'" align="middle" /></td>'.NL.
                      '   <td>'.$counter.'</td>'.NL.
